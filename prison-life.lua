--============================================================
--  Prison Life Enhanced | @nklays  v2.0
--  PC - Keybinds & Floating Buttons
--  Features: Third Person Aimbot, Prediction, Wall Check,
--            Animated UI, Movement Mods, Crosshair & More
--============================================================
...
if _G.AimbotFOVFill then pcall(function() _G.AimbotFOVFill:Remove() end) end
...
for _, v in ipairs(game:GetService("CoreGui"):GetChildren()) do
    if v.Name == "PH5_Main" or v.Name == "PH5_Lock" or v.Name == "PH5_HUD"
       or v.Name == "PH5_KillFlash" or v.Name == "PH5_Floating"
       or v.Name == "PH5_KeybindModal" or v.Name == "PH5_Crosshair" then
        v:Destroy()
    end
end
...
local S = {
    ...
    AimMode       = "Instant",         -- "Instant" | "Smooth"
    AimCamMode    = "FirstPerson",     -- "FirstPerson" | "ThirdPerson"
    AimSmooth     = 0.15,
    AimPrediction = 0,                 -- 0 to 0.2
    AimWallCheck  = false,
    AimToggle     = false,             -- toggle vs hold
    AimTeam       = "Guards",
...
local LOCATIONS = {
    ...
    Yard     = Vector3.new(420, 98, 2350),
    Armory   = Vector3.new(620, 98, 2500),
    Tower1   = Vector3.new(480, 130, 2400),
    Tower2   = Vector3.new(590, 130, 2400),
    Gate     = Vector3.new(550, 98, 2450),
    Parking  = Vector3.new(700, 98, 2600),
}
...
kbBox.Size = UDim2.new(0,360,0,180)
kbBox.Position = UDim2.new(0.5,-180,0.5,-90)
kbBox.BackgroundColor3 = C.bg
kbBox.BorderSizePixel = 0
kbBox.Parent = kbOverlay
Instance.new("UICorner", kbBox).CornerRadius = UDim.new(0,12)
local kbStroke = Instance.new("UIStroke", kbBox)
kbStroke.Color = C.accent; kbStroke.Thickness = 2

local kbTitle = Instance.new("TextLabel")
kbTitle.Size = UDim2.new(1,-20,0,30)
kbTitle.Position = UDim2.new(0,10,0,14)
...
kbSub.Position = UDim2.new(0,10,0,48)
...
kbHint.Size = UDim2.new(1,-20,0,50)
...
kbCurrent.Position = UDim2.new(0,10,1,-30)
...
local function openKeybindModal(label)
    ...
    kbBox.Size = UDim2.new(0,360,0,0)
    TweenService:Create(kbBox, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
    ...
    kbWaitConn = UIS.InputBegan:Connect(function(input, processed)
        ...
        if input.UserInputType == Enum.UserInputType.Keyboard then
            if input.KeyCode == Enum.KeyCode.Escape then
                closeKeybindModal(); return
            end
            local k = keyToString(input)
            if k then bindKey(kbActiveLabel, k); closeKeybindModal() end
        end
    end)
end
...
conn.globalKeys = UIS.InputBegan:Connect(function(input, processed)
    ...
    if kbOverlay.Visible then return end
    local k = keyToString(input)
...
local function makeFloatDraggable(frame, onMove)
    ...
    frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging = true; startPos = frame.Position; startMouse = UIS:GetMouseLocation()
        end
    end)
    frame.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging = false; if onMove then onMove() end
        end
    end)
...
local function createFloatingButton(label)
    ...
    if savedPos then
        ...
        btn.Position = UDim2.new(0.5, -60, 0.5, -19)
    end
    ...
    local pressed, pressPos, dragged = false, nil, false
    local DRAG_THRESH = 6
    btn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            pressed = true; dragged = false; pressPos = input.Position
        elseif input.UserInputType == Enum.UserInputType.MouseButton2 then
            btn:Destroy(); floatingButtons[label] = nil; S.FloatingButtons[label] = nil
    ...
    UIS.InputChanged:Connect(function(input)
        if pressed and input.UserInputType == Enum.UserInputType.MouseMovement then
            if (input.Position - pressPos).Magnitude > DRAG_THRESH then dragged = true end
        end
    end)
    btn.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            if pressed and not dragged then
                if item.type == "toggle" then
                    item.setState(not item.getState()); updateVisual()
                elseif item.type == "action" then
                    item.fire()
                    btn.BackgroundColor3 = C.green; btn.TextColor3 = Color3.new(1,1,1)
                    task.spawn(function()
                        task.wait(0.3)
...
-- Animated particle background inside the main window
...
local uiParticles = {}
if S.UIBgParticles then
    for i = 1, 25 do
        local p = Instance.new("Frame")
        local sz = math.random(2, 5)
        p.Size = UDim2.new(0, sz, 0, sz)
        p.Position = UDim2.new(math.random() * 0.9 + 0.05, 0, math.random() * 0.9 + 0.05, 0)
        p.BackgroundColor3 = C.accent
        p.BackgroundTransparency = math.random() * 0.5 + 0.5
        p.BorderSizePixel = 0
        p.ZIndex = 0
...
tBarBot.Size = UDim2.new(1, 0, 0, 14)
tBarBot.Position = UDim2.new(0, 0, 1, -14)
...
tLabel.Position = UDim2.new(0, 26, 0, 0)
tLabel.BackgroundTransparency = 1
tLabel.Text = "PRISON LIFE ENHANCED | @nklays"
...
miniBtn.Position = UDim2.new(0, 10, 0.5, -23)
...
closeBtn.MouseButton1Click:Connect(function()
    ...
    pcall(function() _G.AimbotFOVFill:Remove() end)
    for _, c in pairs(conn) do pcall(function() c:Disconnect() end) end
    gui:Destroy(); floatGui:Destroy()
    for _, v in ipairs(game:GetService("CoreGui"):GetChildren()) do
        if v.Name == "PH5_Lock" or v.Name == "PH5_HUD" or v.Name == "PH5_KillFlash"
           or v.Name == "PH5_KeybindModal" or v.Name == "PH5_Crosshair" then
            v:Destroy()
        end
    end
...
div.Size = UDim2.new(0, 1, 1, -42)
div.Position = UDim2.new(0, 120, 0, 42)
...
content.Size = UDim2.new(1, -120, 1, -42)
...
local tabBtns = {}
local activeTab = nil
local tabNames = { "Aimbot", "ESP", "HUD", "Movement", "Teleport", "Settings" }
local tabDots = {
    Aimbot   = C.red,
    ESP      = Color3.fromRGB(0, 170, 255),
    HUD      = C.green,
    Movement = Color3.fromRGB(255, 165, 0),
    Teleport = Color3.fromRGB(255, 159, 67),
    Settings = C.dim,
}
local tabIcons = {
    Aimbot   = "🎯",
...
local function switchTab(name)
    ...
    -- Slide out old page
    local oldPage = activeTab and pages[activeTab] or nil
    activeTab = name
    local newPage = pages[name]

    if oldPage then
        -- Animate old page out (upward)
        TweenService:Create(oldPage, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
            Position = UDim2.new(0, 6, 0, -oldPage.AbsoluteSize.Y)
        }):Play()
        task.delay(0.12, function()
            oldPage.Visible = false
            oldPage.Position = UDim2.new(0, 6, 0, 6)
        end)
    end

    -- Animate new page in (from below)
    newPage.Visible = true
    newPage.Position = UDim2.new(0, 6, 0, content.AbsoluteSize.Y + 12)
...
for i, name in ipairs(tabNames) do
    ...
    indicator.Position = UDim2.new(0, 1, 0.5, -8)
    indicator.BackgroundColor3 = tabDots[name]
...
local function addKeybindButton(card, label)
    ...
    kb.Size = UDim2.new(0, 28, 0, 22)
    kb.Position = UDim2.new(1, -92, 0.5, -11)
    ...
    local function refresh()
        ...
        if k then kb.Text = k; kb.TextColor3 = C.accent; kb.TextSize = 10
        else kb.Text = "..."; kb.TextColor3 = C.dim; kb.TextSize = 14 end
    end
    refresh()
    kb.MouseButton1Click:Connect(function()
        openKeybindModal(label)
        task.spawn(function() while kbOverlay.Visible do task.wait() end; refresh() end)
    end)
    return kb
end
...
local function attachHoldForFloat(element, label)
    ...
    element.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            pressed = true; fireTime = tick()
            task.spawn(function()
                local myTime = fireTime
                task.wait(2)
                if pressed and myTime == fireTime then
                    createFloatingButton(label); pressed = false
                end
            end)
        end
    end)
    element.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then pressed = false end
    end)
end
...
local function mkToggle(par, label, defField, cb, opts)
    ...
    cir.Position = def and UDim2.new(1, -20, 0.5, -8) or UDim2.new(0, 3, 0.5, -8)
    ...
    local function setVisual(v)
        TweenService:Create(bg, TweenInfo.new(0.15), {
            BackgroundColor3 = v and C.green or Color3.fromRGB(55, 55, 65)
        }):Play()
        TweenService:Create(cir, TweenInfo.new(0.15), {
            Position = v and UDim2.new(1, -20, 0.5, -8) or UDim2.new(0, 3, 0.5, -8)
        }):Play()
    end
    ...
    local item = {
        ...
        setState = function(v)
            state = v; setVisual(v); cb(v)
            if item.subscribers then for _, fn in ipairs(item.subscribers) do fn() end end
        end,
        subscribers = {},
    }
    registry[label] = item
...
local function mkDropdown(par, label, opts, defField, cb)
    local card = mkCard(par, 38)
    mkLabel(card, label, UDim2.new(0,12,0,0), 0.4)
    local def = (type(defField) == "string") and S[defField] or defField
    local btn = Instance.new("TextButton")
...
local function mkActionButton(par, label, col, cb, opts)
    ...
    b.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            task.wait(0.08)
            TweenService:Create(b, TweenInfo.new(0.2), { BackgroundColor3 = orig }):Play()
        end)
    end)
    if not opts.noBind then
        local kb = Instance.new("TextButton")
        ...
        local function refresh()
            local k = S.Keybinds[label]
            if k then kb.Text = k; kb.TextSize = 10 else kb.Text = "..."; kb.TextSize = 14 end
        end
        refresh()
        kb.MouseButton1Click:Connect(function()
            openKeybindModal(label)
...
mkDropdown(ap, "Cam Mode", { "FirstPerson", "ThirdPerson" }, "AimCamMode", function(v) S.AimCamMode = v end)
...
mkSection(ap, "Prediction")
mkSlider(ap, "Prediction", 0, 200, math.floor(S.AimPrediction * 1000), 5, function(v) S.AimPrediction = v / 1000 end)
mkToggle(ap, "Wall Check", "AimWallCheck", function(v) S.AimWallCheck = v end)
mkToggle(ap, "Toggle Mode", "AimToggle", function(v) S.AimToggle = v end)
...
mkToggle(ap, "Filled Circle", "FOVFilled", function(v) S.FOVFilled = v end)
...
mkDropdown(ep, "Show Team", { "All", "Guards", "Criminals", "Inmates" }, "ESPTeam", function(v)
    S.ESPTeam = v; destroyAllDrawings(); destroyAllHighlights()
end)
mkSection(ep, "Style")
mkDropdown(ep, "ESP Style", { "Highlight", "2D Box", "Head Only" }, "ESPStyle", function(v)
    S.ESPStyle = v; destroyAllDrawings(); destroyAllHighlights()
end)
...
mkSlider(ep, "Fill Transparency", 0, 100, math.floor(S.ESPFillAlpha * 100), 5, function(v)
    ...
    for _, d in pairs(espHLCache) do if d.hl then d.hl.FillTransparency = v / 100 end end
end)
...
mkToggle(hp, "Show Kills", "ShowKills", function(v) S.ShowKills = v end)
mkToggle(hp, "Show Time", "ShowTime", function(v) S.ShowTime = v end)
...
mkSection(hp, "Crosshair")
mkToggle(hp, "Crosshair Enabled", "CrosshairOn", function(v) S.CrosshairOn = v end)
mkToggle(hp, "Center Dot", "CrosshairDot", function(v) S.CrosshairDot = v end)
mkToggle(hp, "Outline", "CrosshairOutline", function(v) S.CrosshairOutline = v end)
mkSlider(hp, "Size", 2, 20, S.CrosshairSize, 1, function(v) S.CrosshairSize = v end)
mkSlider(hp, "Gap", 0, 20, S.CrosshairGap, 1, function(v) S.CrosshairGap = v end)
mkSlider(hp, "Thickness", 1, 6, S.CrosshairThickness, 1, function(v) S.CrosshairThickness = v end)
mkTextInput(hp, "Crosshair Color", S.CrosshairColor, function(v) S.CrosshairColor = v end)
...
mkSection(mp, "Speed")
mkToggle(mp, "Speed Enabled", "SpeedEnabled", function(v)
    S.SpeedEnabled = v
    if not v then
...
mkSlider(mp, "WalkSpeed", 16, 200, S.SpeedValue, 1, function(v) S.SpeedValue = v end)
mkSection(mp, "Jump")
mkToggle(mp, "Infinite Jump", "InfJump", function(v) S.InfJump = v end)
mkSection(mp, "Utility")
mkToggle(mp, "Anti-AFK", "AntiAFK", function(v) S.AntiAFK = v end)
mkToggle(mp, "Noclip", "Noclip", function(v) S.Noclip = v end)
...
mkActionButton(tp, "Yard", Color3.fromRGB(50, 138, 78), function()
    if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
        LP.Character.HumanoidRootPart.CFrame = CFrame.new(LOCATIONS.Yard)
    end
end)
mkActionButton(tp, "Armory", Color3.fromRGB(138, 110, 50), function()
    if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
        LP.Character.HumanoidRootPart.CFrame = CFrame.new(LOCATIONS.Armory)
    end
end)
mkActionButton(tp, "Guard Tower 1", Color3.fromRGB(110, 90, 138), function()
    if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
        LP.Character.HumanoidRootPart.CFrame = CFrame.new(LOCATIONS.Tower1)
    end
end)
mkActionButton(tp, "Guard Tower 2", Color3.fromRGB(110, 90, 138), function()
    if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
        LP.Character.HumanoidRootPart.CFrame = CFrame.new(LOCATIONS.Tower2)
    end
end)
mkActionButton(tp, "Main Gate", Color3.fromRGB(90, 138, 110), function()
    if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
        LP.Character.HumanoidRootPart.CFrame = CFrame.new(LOCATIONS.Gate)
    end
end)
mkActionButton(tp, "Parking Lot", Color3.fromRGB(100, 100, 120), function()
    if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
        LP.Character.HumanoidRootPart.CFrame = CFrame.new(LOCATIONS.Parking)
    end
end)
...
mkSection(spp, "UI")
mkToggle(spp, "UI Particles", "UIBgParticles", function(v) S.UIBgParticles = v end, {noBind=true})
...
mkActionButton(spp, "Reset Kill Counter", Color3.fromRGB(130, 80, 0), function()
    killCount = 0
    cfgStatus.Text = "Kill counter reset"; cfgStatus.TextColor3 = C.green
end, {noBind=true})

mkActionButton(spp, "Clear All Keybinds", Color3.fromRGB(120, 60, 60), function()
    keybindMap = {}; S.Keybinds = {}
    cfgStatus.Text = "All keybinds cleared"; cfgStatus.TextColor3 = C.green
end, {noBind=true})

mkActionButton(spp, "Clear All Floating Buttons", Color3.fromRGB(120, 60, 60), function()
    for _, btn in pairs(floatingButtons) do btn:Destroy() end
    floatingButtons = {}; S.FloatingButtons = {}
    cfgStatus.Text = "All floating buttons cleared"; cfgStatus.TextColor3 = C.green
end, {noBind=true})

switchTab("Aimbot")

--=====================
-- FOV CIRCLE (outline + fill)
--=====================
...
fovFill.Radius = S.FOVRadius
fovFill.Filled = true
fovFill.Visible = S.FOVVisible and S.FOVFilled
fovFill.Transparency = 0.92
fovFill.NumSides = 64
fovFill.Thickness = 1
fovFill.Color = Color3.new(1, 1, 1)
_G.AimbotFOVFill = fovFill
...
for i = 1, 4 do
    ...
    l.Visible = false; l.Thickness = S.CrosshairThickness; l.Color = hexToC3(S.CrosshairColor)
    crosshairLines[i] = l
end
local crosshairDot = Drawing.new("Circle")
crosshairDot.Visible = false; crosshairDot.Radius = 1; crosshairDot.Filled = true
crosshairDot.Color = hexToC3(S.CrosshairColor); crosshairDot.NumSides = 12
-- Outline lines
local crosshairOutline = {}
for i = 1, 4 do
    local l = Drawing.new("Line")
    l.Visible = false; l.Thickness = S.CrosshairThickness + 2; l.Color = Color3.new(0, 0, 0)
    crosshairOutline[i] = l
end
...
lockBg.Size = UDim2.new(0, 260, 0, 38)
lockBg.Position = UDim2.new(0.5, -130, 0.87, 0)
lockBg.BackgroundColor3 = Color3.fromRGB(10, 10, 14)
lockBg.BackgroundTransparency = 0.2
lockBg.BorderSizePixel = 0
lockBg.Visible = false
lockBg.Parent = lockGui
Instance.new("UICorner", lockBg).CornerRadius = UDim.new(0, 8)
local lockStroke = Instance.new("UIStroke", lockBg)
lockStroke.Color = C.accent; lockStroke.Thickness = 1
...
hudFrame.Size = UDim2.new(0, 160, 0, 70)
...
hudStroke.Color = C.border; hudStroke.Transparency = 0.5
...
for i = 1, 4 do
    local l = Drawing.new("Line")
    l.Visible = false; l.Thickness = 2; l.Color = Color3.new(1, 1, 1)
    hitLines[i] = l
end
...
local function checkHits()
    for _, v in ipairs(playerList) do
        ...
        if prev and cur < prev and v == lockedTarget and (tick() - lastShotTime) <= SHOT_WINDOW then
            if cur <= 0 then killCount = killCount + 1; showKillFlash() else showHitMark() end
            lastShotTime = 0
        end
        healthPrev[v] = cur
    end
...
local function getClosest(mPos)
    ...
    for _, v in ipairs(playerList) do
        ...
        if not isVisibleToCamera(part) then continue end
...
conn.mb1down = UIS.InputBegan:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1 then
        if S.AimToggle then
            ...
            if aimActive then lastShotTime = tick() end
        else
            holding = true
            lastShotTime = tick()
        end
...
local function hideAllDrawings()
    for _, d in pairs(espDrwCache) do
        ...
        d.head.Visible = false; d.tracer.Visible = false; d.name.Visible = false; d.info.Visible = false
    end
end
...
conn.render = RunService.RenderStepped:Connect(function(dt)
    ...
    if S.ShowKills then table.insert(hudParts, "Kills: " .. killCount) end
    if S.ShowTime then table.insert(hudParts, os.date("%H:%M:%S")) end
    hudLabel.Text = table.concat(hudParts, "\n")
    hudFrame.Visible = #hudParts > 0
    hudFrame.Size = UDim2.new(0, 160, 0, 16 * #hudParts + 12)
    ...
    -- FOV Fill
    fovFill.Position = mPos
    fovFill.Radius = S.FOVRadius
    fovFill.Visible = S.FOVVisible and S.FOVFilled
    fovFill.Color = S.FOVColorMode == "Rainbow" and rainbow or hexToC3(S.FOVCustomHex)
    ...
    if S.CrosshairOn then
        ...
        local col = hexToC3(S.CrosshairColor)
        ...
        for i, data in ipairs(crossData) do
            if S.CrosshairOutline then
                crosshairOutline[i].From = data[1]; crosshairOutline[i].To = data[2]
                crosshairOutline[i].Thickness = thick + 2; crosshairOutline[i].Visible = true
            else
                crosshairOutline[i].Visible = false
            end
            crosshairLines[i].From = data[1]; crosshairLines[i].To = data[2]
            crosshairLines[i].Color = col; crosshairLines[i].Thickness = thick; crosshairLines[i].Visible = true
        end

        if S.CrosshairDot then
            crosshairDot.Position = Vector2.new(cx, cy)
            crosshairDot.Color = col; crosshairDot.Visible = true
        else
            crosshairDot.Visible = false
        end
    else
        for _, l in ipairs(crosshairLines) do l.Visible = false end
        for _, l in ipairs(crosshairOutline) do l.Visible = false end
        crosshairDot.Visible = false
    end

    -- Animate UI particles
    if S.UIBgParticles then
        for _, p in ipairs(uiParticles) do
            local f = p.frame
            local x = f.Position.X.Scale + p.vx
            local y = f.Position.Y.Scale + p.vy
            if x < 0 or x > 1 then p.vx = -p.vx; x = math.clamp(x, 0, 1) end
            if y < 0 or y > 1 then p.vy = -p.vy; y = math.clamp(y, 0, 1) end
    ...
    -- AIMBOT
    --=====================
    local aiming = S.AimbotOn and (holding or (S.AimToggle and aimActive))
    ...
    if aiming then
        ...
        if not isValid(lockedTarget) then
            ...
            if part then
                ...
                if S.AimPrediction > 0 then
                    ...
                    if hrp then
                        ...
                        targetPos = targetPos + vel * S.AimPrediction
                    end
                end

                -- Wall check for aimbot target
                if not isVisibleToCamera(part) then
                    lockedTarget = nil; lockBg.Visible = false
                elseif S.AimCamMode == "FirstPerson" then
                    -- FIRST PERSON: Point camera at target
                    ...
                    -- THIRD PERSON: Circle Aimbot (mousemoverel)
                    -- Move the mouse to the target's screen position
                    -- Game thinks you're aiming directly at them
                    local screenPos, onScreen = Camera:WorldToScreenPoint(targetPos)
                    if onScreen then
                        local currentMouse = UIS:GetMouseLocation()
                        local deltaX = screenPos.X - currentMouse.X
                        local deltaY = screenPos.Y - currentMouse.Y
                        -- Use mousemoverel (available in most executors)
                        pcall(function()
            ...
            local modeLabel = S.AimCamMode == "ThirdPerson" and " [3RD]" or " [1ST]"
            lockTxt.Text = "LOCKED >> " .. lockedTarget.Name .. " [" .. tn .. "]" .. modeLabel
            lockTxt.TextColor3 = rainbow
            lockBg.Visible = true
            -- Pulse the lock border
    ...
    if useDrawings then
        ...
        for _, v in ipairs(playerList) do
            ...
            if style == "2D Box" then
                ...
                local cf, size
                if okBB and cfBB then cf, size = cfBB, sizeBB
                else cf = hrp.CFrame; size = Vector3.new(4, 5, 2) end
                ...
                if anyVisible then
                    ...
                    local textY = minY - 4
                    if doNames then
                        d.name.Text = v.Name; d.name.Position = Vector2.new(cx, textY - 14)
                        d.name.Color = tc; d.name.Visible = true; textY = textY - 14
                    end
                    local infoParts = {}
                    ...
                    if #infoParts > 0 then
                        ...
                        d.info.Color = Color3.new(1, 1, 1); d.info.Visible = true
                    end
                end
            ...
            elseif style == "Head Only" then
                if head then
                    ...
                    if hs.Z > 0 then
                        ...
                        d.head.Radius = r; d.head.Color = tc; d.head.Visible = true

                        local textY = hs.Y - r - 4
                        if doNames then
                            d.name.Text = v.Name; d.name.Position = Vector2.new(hs.X, textY - 14)
                            d.name.Color = tc; d.name.Visible = true; textY = textY - 14
                        end
                        local infoParts = {}
                        if doHealth then table.insert(infoParts, math.floor(hum.Health) .. "HP") end
            ...
            if doTracers then
                local okBB, cfBB, sizeBB = pcall(function()
                    local cf, sz = ch:GetBoundingBox(); return cf, sz
                end)
                local bottomWorld
                if okBB and cfBB then bottomWorld = cfBB.Position - Vector3.new(0, sizeBB.Y / 2, 0)
                else bottomWorld = hrp.Position - Vector3.new(0, 3, 0) end
                local fs, _ = Camera:WorldToViewportPoint(bottomWorld)
                if fs.Z > 0 then
                    d.tracer.From = tracerOrigin; d.tracer.To = Vector2.new(fs.X, fs.Y)
                    d.tracer.Color = tc; d.tracer.Visible = true
                end
            end
        end
    end
        ...
        if now - espTick >= 0.05 then
            ...
            for _, v in ipairs(playerList) do
                ...
                if not cache or cache.ch ~= ch then
                    ...
                    hl.Name = "PH5_HL"; hl.Adornee = ch
                    hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
                    hl.FillColor = tc; hl.FillTransparency = S.ESPFillAlpha
                    hl.OutlineColor = Color3.new(1, 1, 1); hl.Parent = ch

                    local anchor = head or hrp
                    local bb = Instance.new("BillboardGui")
                    bb.Name = "PH5_BB"; bb.AlwaysOnTop = true
                    ...
                    lbl.TextColor3 = tc; lbl.TextStrokeTransparency = 0.6
                    lbl.TextStrokeColor3 = Color3.new(0, 0, 0); lbl.Text = ""; lbl.Parent = bb
...
--=====================
-- Speed modifier
conn.speedLoop = RunService.Heartbeat:Connect(function()
    if S.SpeedEnabled then
        local hum = LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")
        if hum and hum.WalkSpeed ~= S.SpeedValue then
